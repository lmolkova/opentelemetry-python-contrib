...
def embeddings_create_v_new(handler: TelemetryHandler):
...
def async_embeddings_create_v_new(handler: TelemetryHandler):
...
def _set_embeddings_response_attributes(
    ...
    if getattr(result, "model", None):
        invocation.attributes.update({GenAIAttributes.GEN_AI_RESPONSE_MODEL: result.model})

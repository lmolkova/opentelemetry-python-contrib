...
class WeaverContainer(DockerContainer):
    ...
    def start(self, timeout: int = 60) -> "WeaverContainer":
        ...
        try:
            ...
            logger.error("Error while waiting for Weaver container to be ready, %s", e)
            raise
        self._ready = True
        return self
    ...
    def _wait_for_ready(self, timeout: int=60) -> None:
        for i in range(timeout):
            try:
                ...
                response = post(f"http://localhost:4320", timeout=5)
                if response.status_code == 404:
                    return
                logger.debug("Weaver live-check container not ready yet, status %s, try %s", response.status_code, i)
            except ReqConnectionError as e:
                logger.debug("Health check exception: %s", e)
                pass
            time.sleep(1)
        raise TimeoutError("Weaver live-check container did not become ready in time")
    ...
    def end_live_check(self, timeout: int = 30) -> dict[str, Any]:
        ...
        try:
            if self._ready:
                response = post(f"http://localhost:4320/stop", timeout=5)
            ...
            error_message = f"violations: {violations}" if violations else f"logs: {logs}"
            raise Exception(f"Exited with non-zero status: {exit_code}, {error_message}")
        except Exception as e:
            self._clean_report_dir = False  # keep report for debugging
            logs = self._read_weaver_logs()
            logger.error("Error during weaver live-check: %s, logs: %s", e, logs)
            raise
